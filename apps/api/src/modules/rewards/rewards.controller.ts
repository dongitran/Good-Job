...
export class RewardsController {
  ...
  disableReward(
    ...
    return this.rewardsService.disableReward(
      user.orgId ?? '',
      user.sub,
      rewardId,
    );
  ...
  @HttpCode(200)
  @Roles(UserRole.ADMIN, UserRole.OWNER)
  enableReward(@CurrentUser() user: JwtPayload, @Param('id') rewardId: string) {
    return this.rewardsService.enableReward(
      user.orgId ?? '',
      user.sub,
      rewardId,
    );
  ...
  async deleteReward(
    ...
    await this.rewardsService.deleteReward(
      user.orgId ?? '',
      user.sub,
      rewardId,
    );
