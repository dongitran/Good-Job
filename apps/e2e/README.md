...
- Before and after each Playwright run, global setup/teardown will clean DB rows generated by E2E users (`e2e.%@example.com`) so test data does not keep accumulating.
...
- `E2E_CLEANUP_EMAIL_PATTERN`: optional SQL `LIKE` pattern for cleanup target users (default: `e2e.%@example.com`).
- `E2E_SKIP_DB_CLEANUP`: set to `true` to disable automatic DB cleanup.
